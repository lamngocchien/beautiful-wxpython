...
def apply_dark_scrollbars(window):
    """Ép Windows sử dụng giao diện Dark Mode cho native controls (như thanh cuộn)."""
...
def apply_dark_titlebar(frame):
    """Ép thanh tiêu đề gốc của cửa sổ Windows sang màu tối."""
    if platform.system() == 'Windows':
        try:
...
class C:
    ...
    ACCENT      = wx.Colour(139, 156, 255)   # indigo
...
def text(gc, s, x, y, font=None, color=None):
    if font:  gc.SetFont(gc.CreateFont(font, color or C.TEXT))
    gc.DrawText(s, x, y)
...
class SidebarPanel(wx.ScrolledWindow):
    ...
    def __init__(self, parent, on_select):
        ...
        # ÁP DỤNG SCROLLBAR DARK MODE
        apply_dark_scrollbars(self)
        
        self.on_select  = on_select
        self.hovered    = None   # ('group',gi) or ('conn',gi,ci)
...
class CodeArea(wx.ScrolledWindow):
    ...
    def _lines(self):
        ...
        return [
            ...
            ([], False),
            ([("connect", C.SYN_KW, False),
              (" '",       C.SYN_STR, False),
              (c['ip'],    C.SYN_IP,  False),
              (":",        C.SYN_OP,  False),
              (c['port'],  C.SYN_NUM, False),
              ("'",        C.SYN_STR, False),
              (" ",        C.TEXT,    False),
              (c['proto'], C.SYN_PARAM, False),
              (" /2",      C.SYN_PARAM, False),
              (" /auth",   C.SYN_PARAM, False),
              ("=",        C.SYN_OP, False),
              (c['auth'],  C.SYN_VAL, False),
              (" /user",   C.SYN_PARAM, False),
              ("=",        C.SYN_OP, False),
              (c['user'],  C.SYN_VAL, False),
              (" /passwd", C.SYN_PARAM, False),
              ("=",        C.SYN_OP, False),
              ("****",     C.SYN_VAL, False),
             ], True),
            ([], False),
...
class StatusBar(wx.Panel):
    ...
    def _paint(self, event):
        ...
        item3_end = mb_x + mb_w
        vline(gc, item3_end, 0, h, SEP_COL)
        ...
        vline(gc, w - tw_t - tw_n - 2, 0, h, SEP_COL)
